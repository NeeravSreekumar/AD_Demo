...
def _loss_function(p_out, p_obs, method="jensen-shannon"):
    if method == "jensen-shannon":
        num = (p_out + p_obs) / 2.0
        js_divergence = entropy(p_out, num) / 2.0 + entropy(p_obs, num) / 2.0
...
class model(quantum_circuit):
    ...
    train()
        Runs the optimization algorithm to update theta.
    ...
    def __init__(self, ncells, genes, theta, edges, p_obs, drop_zero=True,
                 epochs=1000, learning_rate=0.1, method="jensen-shannon",
        ...
        learning_rate : float
            ...
            Default : 0.1
        method : str
            Method for the loss function. Default : jensen-shannon.
        train_encoder : bool
        ...
        self.loss_threshold = loss_threshold or 1e-16
        self.loss_ratio = 1
        self.gradient = None
    ...
    def train(self):
        ...
        circuit model for GRN using the Adam optimizer.
        """
        ...
        for epoch in range(self.epochs):
            ...
            self.optimizer.apply_gradients(zip(self.gradient, self.theta))
            self.generate_circuit()
